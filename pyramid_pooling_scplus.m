function pyramid_pooling_scplus(img_dir, fea_dir, pyramid)
...
for  n = 1:length( database.path )
    ...
    %assert(label0==label);
    code=code_scplus;
    fprintf('pyramid pooling for ssc %d of %d\n', n , length(database.path));
    ...
    feapath = fullfile(fea_dir, num2str(label), [fname(1:end-11) '.mat']);
