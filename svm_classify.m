function [Ravg, Rstd, acct] = svm_classify(fea_dir, tr_num, c_svm, nRounds, mem_block, n_ref)
% -------------------------------------------------------------------------
...
for ii = 1:nRounds,
    ...
    [pathstr,~]=fileparts(fea_dir);
	model_dir=fullfile([pathstr(1:end-4), 'model'], num2str(n_ref), num2str(ii));
    if ~exist(model_dir,'dir')
        mkdir(model_dir);
    end
    model_path=fullfile(model_dir, ['model(', num2str(accuracy(ii)), ').mat']);
    save(model_path,'model','tr_idx','ts_idx');
